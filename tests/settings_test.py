...
SETTINGS_LOAD_FROM = '/tmp/settings'
SETTINGS_SAVE_TO = '/tmp/settings.new'
...
class SettingsTest(unittest.TestCase):
    ...
    def tearDownClass(cls):
        try:
            os.remove(SETTINGS_LOAD_FROM)
        except: pass

        try:
            os.remove(SETTINGS_SAVE_TO)
        except: pass
    ...
    def test_load_non_exists_file_should_raise_io_error(self):
        self.assertRaises(IOError, lambda: s.load('/tmp/notfound'))

    def load_with_conf(self, conf_str):
        with open(SETTINGS_LOAD_FROM, 'w') as f:
            f.write(conf_str)
        s.load(SETTINGS_LOAD_FROM)
    ...
    def test_load_set_save_should_persist_config_on_disk(self):
        ...
        s.save(SETTINGS_SAVE_TO)

        self.conf_equal(SETTINGS_SAVE_TO, [
            '[main]\n',
            'assetdir = dir\n',
