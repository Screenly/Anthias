...
class SettingsTest(unittest.TestCase):
    ...
    def test_default_settings(self):
        with fake_settings(empty_settings) as (mod_settings, settings):
            viewer_defaults = mod_settings.DEFAULTS['viewer']
            self.assertEquals(settings['player_name'],
                              viewer_defaults['player_name'])
            self.assertEquals(settings['show_splash'],
                              viewer_defaults['show_splash'])
            self.assertEquals(settings['shuffle_playlist'],
                              viewer_defaults['shuffle_playlist'])
            self.assertEquals(settings['debug_logging'],
                              viewer_defaults['debug_logging'])
            self.assertEquals(settings['default_duration'],
                              viewer_defaults['default_duration'])
