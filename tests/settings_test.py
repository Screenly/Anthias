...
class SettingsTest(unittest.TestCase):
    ...
    def test_default_settings(self):
        with fake_settings(empty_settings) as (mod_settings, settings):
            self.assertEqual(settings['player_name'], mod_settings.DEFAULTS['viewer']['player_name'])
            self.assertEqual(settings['show_splash'], mod_settings.DEFAULTS['viewer']['show_splash'])
            self.assertEqual(settings['shuffle_playlist'], mod_settings.DEFAULTS['viewer']['shuffle_playlist'])
            self.assertEqual(settings['debug_logging'], mod_settings.DEFAULTS['viewer']['debug_logging'])
            self.assertEqual(settings['default_duration'], mod_settings.DEFAULTS['viewer']['default_duration'])
