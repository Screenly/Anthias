...
def test_resolve_reads_from_redis_cache(bare_metal_no_pending: None) -> None:
    with (
        mock.patch('anthias_server.api.views.v2.is_balena_app', return_value=False),
        mock.patch.object(
...
def test_resolve_balena_reads_supervisor_response() -> None:
    ...
    with (
        mock.patch('anthias_server.api.views.v2.is_balena_app', return_value=True),
        mock.patch(
            'anthias_server.api.views.v2.get_balena_device_info', return_value=fake_response
        ) as m_get,
    ):
        assert v2_views._resolve_node_ip() == _FIXTURE_IPV4
