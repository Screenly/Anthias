...
// The Add modal is hidden with x-show, so the DOM — and the Alpine
// state behind it — outlives a close. Everything the operator left in
// it has to be wiped on the way back in.
describe('the Add modal opens clean', () => {
  test('a second open drops the previous asset URL', () => {
    const app = window.homeApp()
    app.openAdd()
    app.addUri = 'https://engadget.com'
    app.closeModal()
  ...
  test('a second open lands back on the From URL tab', () => {
    const app = window.homeApp()
    app.openAdd()
    app.tab = 'apps'
    app.closeModal()
  ...
  // progress lives on the file tab — reopening to check on it must
  // not bounce the operator to a tab that doesn't show it.
  test('reopening during an upload stays on the file tab', () => {
    const app = window.homeApp()
    app.openAdd()
    app.tab = 'file'
  ...
  // appsTab() owns the catalog/config pane and can't be reached from
  // here, so it resets itself off this event.
  test('opening tells the Apps pane to reset', () => {
    let fired = 0
    const onOpen = (): void => {
      fired += 1
    }
    window.addEventListener('add-modal-open', onOpen)
    try {
      window.homeApp().openAdd()
    } finally {
      window.removeEventListener('add-modal-open', onOpen)
    }
